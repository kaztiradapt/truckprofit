      ...
      <section className="content" id="overview">
        ...
        <section className="board-grid compact-board">
          ...
          <article className="panel" id="drivers"><p className="eyebrow">Команда</p><h2>{data.drivers.length} водителей</h2>
            <ul className="entity-list driver-list">{data.drivers.length ? data.drivers.map((driver) => <li key={driver.id}><span>{driver.displayName}<small>{driver.telegramLinked ? "Telegram подключён" : "Telegram не подключён"}</small></span><span className="driver-actions"><span className="badge">{driver.status}</span>{canOperate && !driver.telegramLinked ? <DriverInviteButton driverId={driver.id} /> : null}</span></li>) : <li className="empty-state">Нет добавленных водителей.</li>}</ul>
          </article>
        ...
        {canOperate ? <section className="operations" id="operations">
          ...
          <div className="workbench">
            ...
            <form action={createDriver} className="flow-card"><b>2. Водитель</b><input type="hidden" name="organization_id" value={data.organization.id} /><input name="display_name" placeholder="Марат Садыков" required /><span className="flow-hint">Ссылка Telegram появится в карточке водителя выше.</span><button type="submit">Сохранить</button></form>
