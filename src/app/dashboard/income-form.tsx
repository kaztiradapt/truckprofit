...
export function IncomeForm({ organizationId, baseCurrency, trips }: IncomeFormProps) {
  ...
  const [currency, setCurrency] = useState<SupportedCurrency>(baseCurrencyCode);
  const needsFxRate = currency !== baseCurrencyCode;

  return (
    <form action={createIncome} className="flow-card">
      <div className="flow-card-heading"><span className="flow-step">2</span><span><b>Доход</b><small>Оплата от заказчика · учёт в {baseCurrencyCode}</small></span></div>
      ...
      <div className="flow-fields income-fields">
        ...
        {needsFxRate ? <label className="flow-field flow-field-wide fx-rate-field"><span>Курс к {baseCurrencyCode}</span><input name="fx_rate" inputMode="decimal" placeholder={`1 ${currency} = сколько ${baseCurrencyCode}`} aria-describedby="income-fx-help" required /><small id="income-fx-help">Введите курс на дату договорённости с заказчиком. Сумма автоматически пересчитается в {baseCurrencyCode} для P&amp;L.</small></label> : <input type="hidden" name="fx_rate" value="1" />}
