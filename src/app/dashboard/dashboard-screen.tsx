...
export async function DashboardScreen({ section, searchParams }: { section: DashboardSection; searchParams: Promise<{ error?: string; message?: string }> }) {
  ...
  const canManageTeam = data.permissions.includes("MANAGE_TEAM");
  ...
  const navigationItems = [
    ...
    { href: "/dashboard/team", label: "Сотрудники", visible: canManageTeam, key: "team" },
      ...
      <section className="content">
        ...
        {section === "help" ? <section className="help-section">
          ...
          <div className="help-grid">
            <article className="panel"><h3>Владельцу</h3><ol><li>Добавьте автомобиль в разделе «Автомобили», а водителя — в разделе «Водители».</li><li>Откройте «Изменить» у водителя и закрепите за ним автомобиль. При выборе водителя в новом рейсе эта машина подставится сама.</li><li>В разделе «Сотрудники» можно добавить управляющих и совладельцев. Совладельцу обязательно укажите email для входа в кабинет; Telegram @тег нужен для доступа к боту.</li><li>В «Создании рейса» найдите адрес погрузки и выгрузки. Если нужен конкретный дорожный коридор, выберите «Через» и найдите промежуточный город или точку на карте.</li><li>Выберите подходящий вариант и проверьте плановый километраж: он подставляется с карты, но его можно исправить вручную. Выбранная линия сохранится вместе с рейсом.</li><li>В разделе «Рейсы» нажмите «Подробнее» у любой активной машины: внутри будут карта, геопозиции, километраж и расходы именно этого рейса.</li><li>Сводку по рейсам и пробегу команды смотрите в разделе «Отчёты»; там можно отфильтровать данные по водителю и автомобилю.</li><li>Расходы водителей учитываются автоматически, а после закрытия рейса можно рассчитать P&amp;L.</li></ol></article>
          ...
          <div className="faq-list">
            <details><summary>Как скачать отчёт на компьютер или телефон?</summary><p>В разделе «Отчёты» выберите период, водителя и автомобиль, затем нажмите «Скачать PDF» или «Скачать CSV». PDF — готовый документ, CSV можно открыть в Excel. В Mini App файл отправляется в ваш личный чат с ботом; оттуда его можно сохранить на устройство. Если скачивание в обычном браузере заблокировано, используйте «PDF в Telegram» или «CSV в Telegram». Telegram должен быть подключён к вашему профилю. Между отправками предусмотрена пауза 15 секунд.</p></details>
            <details><summary>Можно ли назначить водителя после создания рейса?</summary><p>Да. Плановый доход от заказчика не мешает назначению. Машину, водителя, дату и тип пробега можно менять до появления фактических данных: расхода, одометра, статуса водителя или геометки. После начала работы эти поля блокируются для сохранения истории учёта.</p></details>
            <details><summary>В какой валюте водитель вводит расходы?</summary><p>Выберите валюту чека: KZT, RUB, USD, CNY или UZS. Если она отличается от валюты компании, бот запросит курс: сколько единиц валюты компании за 1 единицу валюты чека. Курс указывается вручную на дату расхода. Сохраняются исходная сумма, курс и итог для учёта. Перед сохранением доступны «Назад» и исправление полей.</p></details>
            <details><summary>Как исправить расход и открыть чек в боте?</summary><p>Водитель открывает «Мои расходы» и выбирает запись текущего рейса. Там можно посмотреть приложенные чеки и последние 10 исправлений. В активном рейсе можно изменить сумму, валюту, курс и литры своего расхода из Telegram. Категория и одометр уже записанного расхода не изменяются; закрытые рейсы защищены от исправлений водителем.</p></details>
            <details><summary>Как водитель принимает назначенный рейс?</summary><p>В «Мой рейс» нажмите «Принять рейс» или «Не могу принять» и укажите причину. Ответ виден владельцу в карточке рейса. Принятие не включает статус «В пути» автоматически. Отказ не удаляет рейс — владелец решает, кому его переназначить. После смены водителя или машины старые кнопки принятия недействительны.</p></details>
            ...
            <details><summary>Чем совладелец отличается от основного владельца?</summary><p>Совладелец получает полный рабочий кабинет, финансы, рейсы, транспорт, отчёты и управление обычными сотрудниками. Только основной владелец может назначать и удалять совладельцев; его собственную запись изменить или удалить нельзя.</p></details>
            ...
            <details><summary>Откуда берётся километраж рейса?</summary><p>После выбора погрузки и выгрузки кабинет строит автомобильный маршрут и подставляет его расстояние. Для маршрута через определённый город выберите точку «Через» — система перестроит линию, время и километраж с учётом неё. Перед созданием рейса значение можно исправить вручную.</p></details>
            ...
            <details><summary>О чём можно спрашивать ИИ‑помощника?</summary><p>В разделе «Отчёты» можно своими словами спрашивать о рейсах, автомобилях, водителях, доходах, расходах, топливе, пробеге, прибыли и работе функций TruckProfit. Помощник отвечает по выбранным фильтрам и ничего не меняет в учёте. Погода, новости и другие внешние темы отключены; чеки, контакты, Telegram‑ID и геопозиции автоматически из базы в модель не передаются.</p></details>
