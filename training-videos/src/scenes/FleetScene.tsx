        ...
        {!showDriver ? <>
          <section style={{ padding: 20, border: "1px solid #d9e2dd", borderRadius: 14, backgroundColor: "#ffffff" }}>
            ...
            <div style={{ marginTop: 20, display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
              <Field label="МАРКА И МОДЕЛЬ" value="Volvo FH" active={frame >= 82 && frame < 145} />
              <Field label="ГОС. НОМЕР" value="123 ABC 10" active={frame >= 145 && frame < 200} />
          ...
          <GuidedCursor stops={[
            { frame: 82, x: 390, y: 205, click: true, label: "Марка и модель" },
            { frame: 148, x: 690, y: 205, click: true, label: "Гос. номер" },
            { frame: 205, x: 345, y: 335, click: true, label: "Добавить автомобиль" },
            { frame: 260, x: 105, y: 262, click: true, label: "Открыть водителей" },
          ]} />
        </> : <>
          <section style={{ padding: 20, border: "1px solid #d9e2dd", borderRadius: 14, backgroundColor: "#ffffff" }}>
            ...
            <div style={{ marginTop: 20, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <Field label="ИМЯ ВОДИТЕЛЯ" value="Иван Петров" active={frame >= 320 && frame < 395} />
              <Field label="ЗАКРЕПЛЁННЫЙ АВТОМОБИЛЬ" value="Volvo FH · 123 ABC 10" active={frame >= 395 && frame < 465} />
            </div>
          ...
          <GuidedCursor stops={[
            { frame: 320, x: 475, y: 205, click: true, label: "Имя водителя" },
            { frame: 398, x: 875, y: 205, click: true, label: "Закрепить автомобиль" },
            { frame: 470, x: 345, y: 335, click: true, label: "Добавить водителя" },
          ]} />
        </>}
      </AppWindow>
    </SceneBase>
