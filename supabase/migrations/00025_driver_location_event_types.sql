-- Store the location event type selected by the driver in Telegram together with the point.
...
drop function if exists public.record_telegram_trip_location(
  uuid, uuid, uuid, double precision, double precision, numeric, timestamptz, bigint, text, text
);
...
create or replace function public.record_telegram_trip_location(
  ...
  p_event_type text default 'CHECKPOINT'
)
returns uuid
language plpgsql
...
begin
  ...
  if p_latitude is null or p_latitude < -90 or p_latitude > 90
    ...
    or p_event_type not in ('CHECKPOINT', 'REST', 'LOADING', 'UNLOADING', 'OTHER')
    or (normalized_note is not null and char_length(normalized_note) > 300)
    or (p_event_type = 'OTHER' and normalized_note is null) then
